...
-- Create temporary tables for loading data from CSV files
...
COPY temp_users FROM '/csvs/categories.csv' WITH CSV HEADER;
...
-- Load data from temporary tables into OLTP tables
...
-- UserProfiles Table with Cleaning
...
-- Categories Table with Cleaning
...
-- FlavorProfiles Table with Cleaning
...
BEGIN
    FOR order_rec IN SELECT * FROM temp_orders
        ...
        -- Check whether the order was just created or already existed (v_order_id != null), insert order details
...
-- ProductSuppliers Table with Cleaning
...
BEGIN
    FOR wishlist_rec IN SELECT * FROM temp_wishlists
        ...
        -- Insert Wishlist and get wishlist_id
        ...
        -- Check whether the wishlist was just created or already existed (v_wishlist_id != null), insert wishlist details
...
-- ProductViews Table with Cleaning
